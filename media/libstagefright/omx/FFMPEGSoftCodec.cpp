...
status_t FFMPEGSoftCodec::setVideoFormat(
        status_t status,
        const sp<AMessage> &msg, const char* mime, sp<IOMXNode> OMXhandle,
        bool isEncoder,
    ...
    if (status != OK) {
        ...
        if (isEncoder) {
            ...
            if (strncmp(componentName, "OMX.ffmpeg.", 11) == 0) {
                err = setWMVFormat(msg, OMXhandle);
                if (err != OK) {
        ...
        // compressionFormat will be override later
            ...
            *compressionFormat = (OMX_VIDEO_CODINGTYPE)OMX_VIDEO_CodingHEVC;
            ...
            err = setFFmpegVideoFormat(msg, OMXhandle);
            if (err != OK) {
    ...
    if (err == OK && (!strncmp(componentName, "OMX.qcom.", 9)
        ...
        if (!strcasecmp(MEDIA_MIMETYPE_VIDEO_DIVX, mime) ||
            ...
            *compressionFormat = (OMX_VIDEO_CODINGTYPE)QOMX_VIDEO_CodingDivx;
            setQCDIVXFormat(msg, mime, OMXhandle, kPortIndexOutput);
        }
        ...
        if (msg->findInt32("thumbnail-mode", &thumbnailMode) &&
            ...
            if (err != OK) {
                ...
                err = OMXhandle->setParameter(indexType,
        ...
        const sp<AMessage> &msg, sp<IOMXNode> OMXhandle)
{
    ...
    status_t err = setRawAudioFormat(msg, OMXhandle);
    if (err != OK)
        return err;
        ...
        const sp<AMessage> &msg, sp<IOMXNode> OMXhandle)
{
    ...
    err = OMXhandle->getParameter(
            (OMX_INDEXTYPE)OMX_IndexParamAudioAlac, &param, sizeof(param));
    if (err != OK)
        return err;
